// plot.rs
//
// Copyright (c) 2021-2025  Douglas P Lau
//
use crate::domain::BoundDomain;
use crate::point::{IntoPoint, Point};
use crate::text::Label;
use hatmil::svg::{G, Path};

/// Plot Type
#[derive(Clone, Copy, Debug)]
pub enum PlotType {
    /// Stacked area plot
    Area,
    /// Line plot
    Line,
    /// Scatter plot
    Scatter,
}

/// Plot for rendering data
pub struct Plot<'a, P>
where
    P: IntoPoint,
{
    /// Plot type
    plot_tp: PlotType,
    /// Values name
    name: &'a str,
    /// Number within chart
    num: u32,
    /// Domain bound to rectangle
    domain: BoundDomain,
    /// Data values
    data: &'a [P],
    /// Label settings
    label: Option<Label>,
}

impl<'a, P> Plot<'a, P>
where
    P: IntoPoint,
{
    /// Create new plot
    fn new(tp: PlotType, name: &'a str, data: &'a [P]) -> Self {
        Plot {
            plot_tp: tp,
            name,
            num: 0,
            domain: BoundDomain::default(),
            data,
            label: None,
        }
    }

    /// Create a new area plot
    pub fn area(name: &'a str, data: &'a [P]) -> Self {
        Plot::new(PlotType::Area, name, data)
    }

    /// Create a new line plot
    pub fn line(name: &'a str, data: &'a [P]) -> Self {
        Plot::new(PlotType::Line, name, data)
    }

    /// Create a new scatter plot
    pub fn scatter(name: &'a str, data: &'a [P]) -> Self {
        Plot::new(PlotType::Scatter, name, data)
    }

    /// Get plot name
    pub(crate) fn name(&self) -> &str {
        self.name
    }

    /// Get mutable plot number
    pub(crate) fn num_mut(&mut self) -> &mut u32 {
        &mut self.num
    }

    /// Get mutable domain
    pub(crate) fn domain_mut(&mut self) -> &mut BoundDomain {
        &mut self.domain
    }

    /// Add labels to plot
    pub fn label(mut self) -> Self {
        self.label = Some(Label::new());
        self
    }

    /// Display a plot
    pub(crate) fn display<'p>(&self, g: &'p mut G<'p>) {
        g.class("plot");
        match self.plot_tp {
            PlotType::Area => self.display_area(g),
            PlotType::Line => self.display_line(g),
            PlotType::Scatter => self.display_scatter(g),
        }
    }

    /// Display an area plot
    fn display_area<'p>(&self, g: &'p mut G<'p>) {
        let cls = format!("plot-{} plot-area", self.num);
        let mut d = Path::def_builder();
        if let Some(pt) = self.data.first() {
            let pt: Point = (*pt).into();
            let x = self.domain.x_map(pt.x);
            let y = self.domain.y_map(0.0);
            d.move_to((x, y));
        }
        for pt in self.data.iter() {
            let pt: Point = (*pt).into();
            let x = self.domain.x_map(pt.x);
            let y = self.domain.y_map(pt.y);
            d.line((x, y));
        }
        if let Some(pt) = self.data.last() {
            let pt: Point = (*pt).into();
            let x = self.domain.x_map(pt.x);
            let y = self.domain.y_map(0.0);
            d.line((x, y));
        }
        g.path().class(cls).d::<String>(d.into()).close();
        self.display_labels(g);
    }

    /// Display a line plot
    fn display_line<'p>(&self, g: &'p mut G<'p>) {
        let cls = format!("plot-{} plot-line", self.num);
        let mut d = Path::def_builder();
        for (i, pt) in self.data.iter().enumerate() {
            let pt = (*pt).into();
            let x = self.domain.x_map(pt.x);
            let y = self.domain.y_map(pt.y);
            if i == 0 {
                d.move_to((x, y));
            } else {
                d.line((x, y));
            }
        }
        g.path().class(cls).d::<String>(d.into()).close();
        self.display_labels(g);
    }

    /// Display a scatter plot
    fn display_scatter<'p>(&self, g: &'p mut G<'p>) {
        let cls = format!("plot-{} plot-scatter", self.num);
        let mut d = Path::def_builder();
        for (i, pt) in self.data.iter().enumerate() {
            let pt = (*pt).into();
            let x = self.domain.x_map(pt.x);
            let y = self.domain.y_map(pt.y);
            if i == 0 {
                d.move_to((x, y));
            } else {
                d.line((x, y));
            }
        }
        g.path().class(cls).d::<String>(d.into()).close();
        self.display_labels(g);
    }

    /// Display plot labels
    fn display_labels<'p>(&self, g: &'p mut G<'p>) {
        if let Some(_label) = &self.label {
            // FIXME: use Label attributes (precision, etc.)
            let mut text = g.text();
            text.class("plot-label");
            for pt in self.data.iter() {
                let pt: Point = (*pt).into();
                let x = self.domain.x_map(pt.x);
                let y = self.domain.y_map(pt.y);
                let lbl = format!("({} {})", pt.x, pt.y);
                text.tspan().x(x).y(y).dy("-0.66em").cdata(lbl).close();
            }
            text.close(); // text
        }
        g.close();
    }
}
